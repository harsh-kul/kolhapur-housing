...
// $host = 'localhost';
// $database = 'housing-kolhapur-db';
// $username = 'housing-admin';
// $password = 'j%@c[_QKsvX-';
...
$query = "INSERT INTO tbl_staff (pk_sfid, sf_name, sf_email, sf_mobile, sf_status, sf_is_deleted, sf_created_at, sf_updated_at, sf_col1, sf_col2, sf_col3, sf_col4, deleted_on, triggered_on) VALUES";
$reg_query = "INSERT INTO `tbl_registration` (`pk_rgid`, `fk_sfid`, `rg_fname`, `rg_lname`, `rg_email`, `rg_mobile`, `rg_address`, `rg_password`, `rd_is_super_admin`, `rg_is_seller`, `rg_is_buyer`, `rg_is_app_user`, `rg_is_deleted`, `rg_created_at`, `rg_updated_at`, `rg_status`, `rg_col1`, `rg_col2`, `rg_col3`, `rg_col4`, `rg_col5`, `rg_deleted_on`, `rg_triggered_on`) 
VALUES";
$pro_query ="INSERT INTO `tbl_property` (
    `fk_usid`,
    `fk_ptid`,
    `pp_price`,
    `pp_plot_no`,
...
for ($i = 1; $i < 200; $i++) {
    $f_name =getRandomIndianFirstName();
    $f_phone =getRandomIndianPhoneNumber();
    $values = "
    ($i, '$f_name', '.$f_name.@gmail.com', '".getRandomIndianPhoneNumber()."', 'Active', 0, '".generateRandomDateTime()."', '".generateRandomDateTime()."', 'Value1', 'Value2', 'Value3', 'Value4', '".generateRandomDateTime()."', '".generateRandomDateTime()."')";
    $reg_values = "($i, $i, '$f_name', '$f_name', '".$f_name."@gmail.com', '$f_phone', '".getRandomLandmark()."', 'password123', 'No', '".getRandomYesNo()."', '".getRandomYesNo()."', 'NO', 0, '".generateRandomDateTime()."', '".generateRandomDateTime()."', 'Active', 'Value1', 'Value2', 'Value3', 'Value4', 'Value5', '".generateRandomDateTime()."', '".generateRandomDateTime()."')";
    ...
    $pro_value ="(
        ...
        'ward".generateRandomNumber(1,200)."',  
        'Building ".generateRandomNumber(1,500)."',
        '".generateRandomNumber(1,1000)." Main St',  
        'Near Park',  
        'CityName',  -- Replace with the actual value for `pp_city`
        'DistrictName',  -- Replace with the actual value for `pp_district`
        'StateName',  -- Replace with the actual value for `pp_state`
        '".generateRandomNumber(1,100).generateRandomNumber(1,100).generateRandomNumber(1,100)."',  -- Replace with the actual value for `pp_pincode`
        '$f_name',  -- Replace with the actual value for `pp_owner_name`
        '$f_phone',  -- Replace with the actual value for `pp_contact_no`
        '".generateRandomNumber(1,7)."',  -- Replace with the actual value for `pp_status`
        '',  -- Replace with the actual value for `pp_rj_reason`
        0,  -- Replace with the actual value for `pp_rj_count`
        '".getRandomPrice()."',  -- Replace with the actual value for `pp_deposite`
        'January',  -- Replace with the actual value for `pp_agreement_month`
        0,  -- Replace with the actual value for `pp_is_deleted`
        '".generateRandomDateTime()."',  -- Replace with the actual value for `pp_created_at`
        '".generateRandomDateTime()."',  -- Replace with the actual value for `pp_updated_at`
        'Col1Value',  -- Replace with the actual value for `pp_col1`
        'Col2Value',  -- Replace with the actual value for `pp_col2`
        'Col3Value',  -- Replace with the actual value for `pp_col3`
        'Col4Value',  -- Replace with the actual value for `pp_col4`
        'Col5Value',  -- Replace with the actual value for `pp_col5`
        '".generateRandomNumber(1,200)."',  -- Replace with the actual value for `pp_soid`
        '".generateRandomDateTime()."',  -- Replace with the actual value for `deleted_on`
        '".generateRandomDateTime()."'  -- Replace with the actual value for `triggered_on`
      )";
    ...
    if ($i < 199) {
        ...
        $reg_values .= ', ';
        $pro_values .= ', ';
         // Add a comma to separate values for multiple records
    }

    $query .= $values;
    $reg_query.=$reg_values;
    $pro_query.=$pro_value;

}

if (mysqli_query($connection, $query) && mysqli_query($connection, $reg_query)&& mysqli_query($connection, $pro_query) ) {
...
function generateRandomDateTime() {
    // Generate random values for year, month, day, hour, minute, and second
    $randomYear = rand(2023, 2024); // You can adjust the range as needed
    ...
    $randomDateTime = new DateTime(sprintf("%04d-%02d-%02d %02d:%02d:%02d", $randomYear, $randomMonth, $randomDay, $randomHour, $randomMinute, $randomSecond));
...
function getRandomYesNo() {
    $randomNumber = rand(0, 1); // Generates a random number 0 or 1
    return $randomNumber === 0 ? 'N' : 'Y';
}

function getRandomIndianPhoneNumber() {
    $prefixes = ["7", "8", "9"]; // Valid mobile prefixes in India
    $randomPrefix = $prefixes[array_rand($prefixes)]; // Select a random prefix

    // Generate random 9 digits for the phone number
    $randomDigits = '';
    for ($i = 0; $i < 9; $i++) {
        $randomDigits .= mt_rand(0, 9);
    }

    $phoneNumber = $randomPrefix . $randomDigits;
    return $phoneNumber;
}
...
function generateRandomNumber($min, $max) {
    if ($min >= 0 && $max >= $min) {
        ...
        return "Invalid input. Make sure min is less than or equal to max.";
    }
}
function getRandomIndianFirstName() {
    ...
    $randomIndex = rand(0, count($indianFirstNames) - 1);

    return $indianFirstNames[$randomIndex];
}
function getRandomIndianLastName() {
    $indianLastNames = [
        "Patel", "Sharma", "Verma", "Gupta", "Singh", "Kumar", "Joshi", "Chauhan", "Agarwal", "Mehta",
        // Add more Indian last names to the list as needed
    ];

    $randomLastName = $indianLastNames[array_rand($indianLastNames)]; // Get a random last name
...
function getRandomIndianState() {
    ...
    $randomState = $indianStates[array_rand($indianStates)];
...
function getRandomIndianCity() {
    $indianCities = array(
        ...
        "Vadodara", "Ghaziabad", "Ludhiana", "Agra", "Nashik", "Rajkot",
        "Meerut", "Varanasi", "Srinagar", "Dhanbad", "Amritsar", "Allahabad"
        // Add more city names as needed
    );

    // Get a random index from the array
    $randomIndex = array_rand($indianCities);

    // Retrieve and return the random Indian city
    $randomCity = $indianCities[$randomIndex];
    return $randomCity;
}
function getRandomLandmark() {
    ...
    $randomIndex = array_rand($landmarks);
    return $landmarks[$randomIndex];
}
function getRandomPrice($min = 1, $max = 100, $decimalPlaces = 2) {
    $randomPrice = number_format((float)rand($min * 100, $max * 100) / 100, $decimalPlaces, '.', '');
    return  $randomPrice;
}
